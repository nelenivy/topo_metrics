...
def main():
    ...
    for params in grid_search_params:
        ...
        result, user_embeddings, item_embeddings, recs = train_and_evaluate(
            'bpr', train_csr, test, params)
        results.append(result)
        pd.DataFrame(results).to_csv(os.path.join(SAVE_PATH, 'ml20m_bpr_grid.csv'), index=False)

        filename = "_".join([f"{k}_{v}" for k, v in sorted(params.items())])
        if SAVE_EMBEDS:
            os.makedirs(os.path.join(SAVE_PATH, 'embeds/bpr/ml20m/users'), exist_ok=True)
            os.makedirs(os.path.join(SAVE_PATH, 'embeds/bpr/ml20m/items'), exist_ok=True)
            np.save(os.path.join(SAVE_PATH, 'embeds/bpr/ml20m/users', filename + ".npy"),
                    user_embeddings)
            np.save(os.path.join(SAVE_PATH, 'embeds/bpr/ml20m/items', filename + ".npy"),
                    item_embeddings)
        if SAVE_RECS:
            os.makedirs(os.path.join(SAVE_PATH, 'recs/bpr/ml20m'), exist_ok=True)
            recs.to_csv(os.path.join(SAVE_PATH, 'recs/bpr/ml20m', filename + ".csv"),
                        index=False)
