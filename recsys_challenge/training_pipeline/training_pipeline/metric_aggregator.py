...
class MetricsAggregator:
    ...
    def _find_best_weighted_metrics_and_epochs(self):
        ...
        for task, metric_tracker in self._aggregated_metrics.items():
            ...
            tracker = metric_tracker[best_epoch]
            self.roc_auc[task.value] = tracker.val_auroc

            if task.value != 'churn':
                self.diversity[task.value] = tracker.val_diversity
                self.novetly[task.value] = tracker.val_novelty
