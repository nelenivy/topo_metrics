...
def run_tasks(
    ...
    if clearml_task:
        ...
        metrics_aggregator._find_best_weighted_metrics_and_epochs()

        for key, value in metrics_aggregator._best_weighted_metrics.items():
            clearml_logger.report_single_value(key, value)

        for key, value in metrics_aggregator.roc_auc.items():
            clearml_logger.report_single_value(f'roc_auc {key}', value)

        for key, value in metrics_aggregator.diversity.items():
            clearml_logger.report_single_value(f'diversity {key}', value)

        for key, value in metrics_aggregator.novetly.items():
            clearml_logger.report_single_value(f'novetly {key}', value)
            
        clearml_logger.report_single_value(
            'sum_of_scores', sum(metrics_aggregator._best_weighted_metrics.values()))

        best_metrics = pd.Series(metrics_aggregator._best_weighted_metrics).to_frame()
        best_epochs = pd.Series(metrics_aggregator._best_epochs).to_frame()

        clearml_logger.report_table(title='best_metrics', series='dataframe',
                                    table_plot=best_metrics)
        clearml_task.upload_artifact('best_metrics', best_metrics)
        clearml_logger.report_table(title='best_epochs', series='dataframe',
                                    table_plot=best_epochs)
        clearml_task.upload_artifact('best_epochs', best_epochs)
